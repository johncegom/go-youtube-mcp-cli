...
A YouTube transcript/metadata/download tool for AI agents and CLI users, exposed both as an MCP stdio server and a standalone CLI, with no YouTube API key — metadata comes from scraping the watch-page HTML and transcripts/downloads come from shelling out to `yt-dlp`.

It started as a faithful Go port of [`umbertotancorre/youtube-mcp-cli`](https://github.com/umbertotancorre/youtube-mcp-cli) (a TypeScript project) — that porting effort (Phase 1) is complete. Since then it has grown into a standalone product: Phase 2 (`docs/DECISIONS.md` DECISION-013) deliberately lifted the "no new features beyond the TS version" constraint and added agent-oriented capabilities with no upstream equivalent (transcript caching, windowed retrieval, observable background downloads — see `docs/PLAN.md`'s "Phase 2" section and `README.md`'s Credits section for the full history). New feature work is scoped through the normal task-approval process below, not bounded by upstream parity.
