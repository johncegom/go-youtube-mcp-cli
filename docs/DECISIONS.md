...
- **Context:** `CLAUDE.md` documents `go build`/`go vet`/`go test`/`gofmt` as "the whole local toolchain," with no Makefile or task runner. Retyping the same command combinations (especially the CI-matching sequence: build, vet, test -race, gofmt check) by hand is repetitive. This isn't a port-plan deliverable (no upstream TS behavior to match, nothing that belongs on the `docs/LEDGER.md` index), so it doesn't get a `docs/tasks/<slug>/TASK.md` — per the proportionality section, ceremony there is scoped to the porting plan, not to ancillary dev tooling.
