...
- **Consequences:** `johncegom/youtube-mcp-cli` vs. `umbertotancorre/youtube-mcp-cli` — `CLAUDE.md` links the former as the TS source, `README.md`'s Credits section names the latter as the actual upstream original. Noticed but deliberately left unfixed here (not this decision's scope; flagged to the human separately) — a future doc pass should confirm which is correct and reconcile the two.
