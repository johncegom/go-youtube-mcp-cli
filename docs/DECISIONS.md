...
- **Consequences:** tool count 17 → 18. `Words`/`Speaking rate` inherit whatever `parseVtt` returns, which for auto-caption videos is currently ~3× inflated — a pre-existing, reachable parser bug surfaced by this task's ground-truth capture and logged as BUG-010, **fixed 2026-09-20** (`parseVtt` now keeps one segment per spoken line for rolling-cue files), which corrected the brief's figures automatically. The tool description explicitly steers agents to `get_chapters` + `get_transcript_range` for long videos or targeted questions, since the brief returns the full timed transcript by design. See `docs/tasks/17-video-brief/TASK.md` for the Definition of Done, Test Plan, and smoke-test transcript.
...
- **Consequences:** **inconsistency by design (deliberate scope cut):** `SaveTranscriptFile` (`download_transcript`, `download_transcript_timed`, CLI `transcript --save`), `get_video_brief`, and `search_playlist` still default to `en`. The same non-English video therefore returns its transcript from `get_transcript` but the BUG-011 error from `download_transcript` until a follow-up applies the same resolution there (each has its own output shape, so it is a separate task) — tracked as **task 19** (`docs/tasks/19-language-resolution-remaining-tools/TASK.md`). First omitted-language call per video costs one extra watch-page fetch (the existing `ytInitialPlayerRe` scan takes on the order of a second on a real page). Naming the spoken language inside the BUG-011 error for an explicit `en` was suggested by Advise and left as a follow-up (`TranscriptErrorText` has no context to look it up). See `docs/tasks/18-native-language-fallback/TASK.md`.

- **Update (2026-09-20, task 19 — `docs/tasks/19-language-resolution-remaining-tools/TASK.md`):** the scope cut above is now closed for two of the three surfaces. `download_transcript*`/CLI `transcript --save` (both call `core.SaveTranscriptFileResolved`, which resolves, saves and returns the note for the result text) and `get_video_brief` (`ResolveLanguage` runs *inside* the brief's transcript goroutine so metadata stays concurrent; a `Language:` line appears in its stats block only when auto-detected and not `en`) now resolve an omitted `language`. **`search_playlist` deliberately stays on the plain `en` default** (human decision: minimum improvement only) — its `language` description now says it is not auto-detected for playlists, and its per-video skip line already carries the BUG-011 language hint; the Advise-recommended lazy retry-on-failure design is parked in the `docs/LEDGER.md` Backlog. **Saved transcripts are now named `<title>_<lang>[_timed].md` for every language including `en`** (human decision; `title.md` → `title_en.md` is a deliberate rename for existing English users, so two languages of one video never overwrite each other), with the language component sanitized to `[A-Za-z0-9_-]` because it is an MCP-controlled argument; the saved file's `**Language:**` header line appears only when the language is not `en`, so English file bodies are unchanged.

- **Update (2026-09-27, task 21 — `docs/tasks/21-original-language-resolution/TASK.md`, BUG-013):** the resolution order above ("any `en*` track → `en`") is refined for videos whose **original audio language is known** from the watch page's `audioTracks[].audioTrackId`. On videos with YouTube's auto-dubbing structure exactly one id ends `.4` (`vi.4`, `de-DE.4`, `en-US.4`) — the original audio — and the rest end `.10`; that language `O` was 14/14 equal to yt-dlp's `language` and identical across four viewer locales (`docs/evidence/bug-013/`). Order now: (1) an **uploaded** track in `O` → that track's code; (2) else an **uploaded English** track → `en`; (3) else the **auto** track in `O` → its code; (4) else the previous rules, unchanged. Step 1 preceding step 2 is the human's decision, quoted: "if it has uploaded subtitles of their original language, use it first. If not, use the English." "The English" means the uploaded English track — auto English on a non-English video is the machine-translated request that 429s (BUG-011). When `O` is unknown (no `.4` id, several, malformed) or has no matching track, the previous rules run untouched, so a video without the structure behaves exactly as before. Codes are matched exact > bare-base or region-shaped (`de` / `de-DE` / `es-419`) > other same-base (custom `vi-abc123`); the track's own code is returned (yt-dlp's `--sub-langs` is a full match), except that any English match resolves to the literal `en`, so no English video's filename, header or note changes. **Why the `.4` id and not other fields:** `streamingData`'s `audioIsDefault` track, `hostLanguage` and `requestLanguage` depend on the *viewer's* locale (measured), so they cannot describe the video. **Why the scope stops at the `.4` id (Advise, `docs/eagd-log.md`):** using a video's single auto track as `O` on videos *without* the structure would apply step 1 to them — a behaviour change on videos not in BUG-013, where today's rule was right 19/19; that is left as a separate, undecided question. **Residual risk, stated:** a lone `.4` id that is not the original (say a creator-dubbed track) would produce a confidently wrong language; the only defence is the 14/14 agreement. The `.4` / `.10` meaning is inferred, not documented; if YouTube changes it, resolution falls back to the previous rules.

## DECISION-023: guarded, peek-only orig-first fetch for English, plus a `translated` log line (task 20, BUG-012 follow-up)

- **Where:** `internal/core/transcript.go` (`fetchPlan`, `fetchWithPlan`, `fetchOnce`, `timedtextTranslation`, `formatPlanLog`, `formatTranslatedLog`), `internal/core/language.go` (`captionInfo` memo: `captionInfoFor`, `peekCaptionInfo`, `hasUploadedTrack`, `hasASRTrack`). Task: `docs/tasks/20-guarded-orig-first/TASK.md`. Evidence: `docs/evidence/bug-012/`.
- **Context:** PR #36's retry turns a 429 on plain `en` into the genuine `en-orig` transcript, but it costs a doomed request first and cannot see the case where plain `en` *succeeds* as a machine back-translation (observed live 2026-09-27: plain `en` on `vyIgAO8aCbA` fetched `lang=uk&tlang=en` with no error).
- **Decision:** decide the attempt order **before** the first yt-dlp call, from the watch page's `captionInfo` **only if it is already in memory** (a cold memo gives today's `[L]` + the shipped retry, so no new request or latency, and DECISION-022's playlist scope cut is untouched). Plan: `[L-orig, L]` only when `L`'s base language is `en`, the page lists an auto track with exactly code `L`, and no uploaded track exists for `L` (prefix match, so a variant-coded uploaded track like `UF8uR6Z6KLc`'s counts and errs toward plain `L`); otherwise `[L]`. If every attempt fails the error of the *requested* language is returned (BUG-011 message intact); `L-orig` is never attempted twice. `errors.log` gains `transcript_fetch_plan` (only for multi-attempt plans) and `transcript_fetch_translated` (only when a successful attempt's verbose output shows a timedtext URL with `tlang=`). A success from `L-orig` is cached under `{videoID, L}`.
- **Restricted to English (human decision after 20.0b):** for Vietnamese and German auto-caption videos `<L>-orig` returned text identical to plain `<L>` on 4 of 5 (0.87 on the fifth), and neither ever 429ed, so orig-first there adds an attempt-order rule for no observed failure. Reversible if a non-English video ever 429s on plain `L`.
- **Alternatives considered:** unguarded orig-first (rejected: replaces an uploaded transcript with speech recognition); a yt-dlp `-J` run to read `tlang` (rejected: ~5 s extra per uncached fetch); fetching the watch page from the fetch funnel when cold (rejected: widens DECISION-022, adds ~1 s and a request to every explicit-language call); replacing the language memo with the `captionInfo` memo (rejected: ~24 edits in `language_test.go` for no user-visible gain, so a second small memo sits beside it, ~25 duplicated lines); "keep PR #36 and stop" (a defensible outcome per Advise; not taken because the silent back-translation is a correctness gap the retry cannot see).
- **Consequences:** explicit-language callers and `search_playlist` still pay the doomed request (memo cold), and the retry recovers them. The plan rests on thin, dated evidence (one machine/day; yt-dlp 2026.07.04 for the measurements, 2026.08.19 in the app) and degrades to today's behaviour when its assumption fails. `transcript_fetch_translated` logs `lang=<requested> source=<lang param> tlang=<tlang param>` (a small departure from the draft's `tlang=<src>`, which was ambiguous).
